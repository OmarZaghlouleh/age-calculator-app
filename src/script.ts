...
let button = document.getElementsByClassName("calc-button")[0] as HTMLButtonElement;
button.onclick = async function () {
    ...
    for (let i = 0; i < inputs.length; i++) {
        let inputField = (inputs[i] as HTMLElement).getElementsByClassName("input")[0] as HTMLInputElement;
    ...
    if (!hasError) {
        ...
        for (let i = 0; i < inputs.length; i++) {
            ...
            if (inputs[i].children.length > 2) inputs[i].children[inputs[i].children.length - 1].remove();
        }
        // Show result
        let resultDiv = document.getElementsByClassName("result")[0] as HTMLDivElement;
        let age: number[] = calcDate(`${year}/${month}/${day}`)
        for (let i = 0; i < resultDiv.children.length; i++) {

            if (resultDiv.children[i].className.includes("days")) {
                // Days
                if (resultDiv.children[i].childNodes.length == 2) resultDiv.children[i].childNodes[0].remove();
                resultDiv.children[i].className = "days";
                (resultDiv.children[i] as HTMLElement).style.setProperty("--days-value", age[0].toString())
                ...
                (resultDiv.children[i] as HTMLElement).style.setProperty("--months-value", age[1].toString())
                ...
                (resultDiv.children[i] as HTMLElement).style.setProperty("--years-value", age[2].toString())
...
function getErrorLabel(label: string): HTMLElement {
...
function valDate(date: string): boolean {
...
function calcDate(date: string): number[] {
    // let today = new Date()
